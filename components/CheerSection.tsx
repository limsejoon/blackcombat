...
function toUIComment(c: DBComment, allFighters: Fighter[]): UIComment {
  const fighter = allFighters.find((f) => f.id === c.fighter_slug);
  return {
    id: c.id,
    user: c.author_name,
    avatar: getAvatar(c.author_name),
    message: c.content,
    targetFighter: fighter ? (fighter.nickname || fighter.name) : null,
        ...
        <div className="p-5">
          ...
          <div className="flex gap-2">
            <button
              ...
              style={{ background: "#f0f0f0", color: "#777777", fontFamily: "'Inter', sans-serif", fontWeight: 700 }}
            >
              취소
            </button>
            <button
...
export function CheerSection({
  ...
  const { comments: dbComments, sendComment } = useComments(tournamentId);
  const [donors, setDonors] = useState<DonorStat[]>([]);
  const [likeState, setLikeState] = useState<Record<string, { likes: number; liked: boolean }>>({});
  ...
  useEffect(() => {
    ...
    supabase
      .from("donations")
      .select("donor_nickname, amount")
      .eq("tournament_id", tournamentId)
      .eq("is_hidden", false)
      .then(({ data }) => {
        if (!data) return;
        const map: Record<string, { total: number; count: number }> = {};
        for (const row of data as { donor_nickname: string; amount: number }[]) {
          const key = row.donor_nickname;
          if (!map[key]) map[key] = { total: 0, count: 0 };
          map[key].total += row.amount;
          map[key].count += 1;
        }
        const sorted = Object.entries(map)
          .map(([user, stat]) => ({ user, avatar: getAvatar(user), ...stat }))
  ...
  const uiComments: UIComment[] = dbComments.map((c) => ({
    ...toUIComment(c, allFighters),
    likes: likeState[c.id]?.likes ?? 0,
    liked: likeState[c.id]?.liked ?? false,
  }));

  async function handleSend() {
    if (!input.trim() || isSending) return;
    const fighterObj = allFighters.find((f) => f.nickname === selectedFighter || f.name === selectedFighter);
    setIsSending(true);
    try {
      await sendComment(input.trim(), username || "익명의 팬", fighterObj?.id ?? undefined);
      setInput("");
  ...
  async function handleCheerDonor(donor: DonorStat, msg: string) {
    await sendComment(`@${donor.user} ${msg}`, username || "익명의 팬");
    setTimeout(() => listRef.current?.scrollTo({ top: 0, behavior: "smooth" }), 50);
  }

  function toggleLike(id: string) {
    setLikeState((prev) => {
      const cur = prev[id] ?? { likes: 0, liked: false };
      return {
        ...prev,
        [id]: { likes: cur.liked ? cur.likes - 1 : cur.likes + 1, liked: !cur.liked },
      };
    });
  }

  const filtered = uiComments.filter((c) => {
    if (filter === "전체") return true;
    if (filter === "선수응원") return !!c.targetFighter;
    if (filter === "일반") return !c.targetFighter;
    return true;
  });
    ...
    return (
      ...
      >
        <div
          ...
          style={{ background: "#f5f2eb", border: "1px solid rgba(232,180,0,0.3)" }}
        >
    ...
    <div className="space-y-4">
      ...
      {topDonor && (
        <div
          ...
          style={{ background: "#111111", border: "1px solid rgba(232,180,0,0.5)", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}
        >
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: "#e8b400" }}>
            ...
            <button
              ...
              style={{ background: "linear-gradient(135deg, #e8b400, #f5c800)", color: "#111111", fontFamily: "'Inter', sans-serif", fontWeight: 800, fontSize: "0.85rem" }}
            >
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
          <div className="px-4 pt-4 pb-3 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
            <div className="flex items-center gap-2">
              ...
              <span style={{ fontFamily: "'Inter', sans-serif", fontWeight: 800, fontSize: "0.95rem", color: "#111111" }}>후원자 랭킹</span>
            </div>
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{ background: idx === 0 ? "rgba(232,180,0,0.12)" : "#f5f5f5", border: `1.5px solid ${idx === 0 ? "#e8b400" : "rgba(0,0,0,0.08)"}`, fontSize: "1.1rem" }}
                >
                  {donor.avatar}
                </div>
                ...
                <button
                  ...
                  style={{ background: "#111111", color: "#e8b400", fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: "0.72rem", whiteSpace: "nowrap" }}
                >
                  <Heart size={10} /> 응원
                </button>
              </div>
          ...
          <div className="px-4 py-3" style={{ background: "#f5f2eb", borderTop: "1px solid rgba(232,180,0,0.2)" }}>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                style={{ background: "linear-gradient(135deg, #e8b400, #f5c800)", color: "#111111", fontFamily: "'Inter', sans-serif", fontWeight: 800, fontSize: "0.8rem" }}
              >
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        <div className="p-4 space-y-2" style={{ borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              style={{
                background: isSending || !input.trim() ? "#eeeeee" : "#111111",
                color: isSending || !input.trim() ? "#aaaaaa" : "#e8b400",
                fontFamily: "'Inter', sans-serif",
                fontWeight: 700,
                whiteSpace: "nowrap",
              }}
            >
              <Send size={14} /> {isSending ? "전송 중" : "전송"}
            </button>
          </div>
        </div>
        ...
        <div ref={listRef} className="overflow-y-auto" style={{ maxHeight: 520, scrollbarWidth: "none" }}>
          {filtered.length === 0 ? (
            <div className="p-8 text-center" style={{ color: "#aaaaaa", fontSize: "0.85rem" }}>
              아직 응원 댓글이 없습니다. 첫 번째로 응원해보세요! 🔥
            </div>
          ) : (
            <AnimatePresence initial={false}>
              {filtered.map((c) => (
                <motion.div
                  ...
                  style={{ background: c.targetFighter ? "#fafafa" : "transparent", border: c.targetFighter ? "1px solid rgba(0,0,0,0.06)" : "1px solid transparent" }}
                >
                  <div className="flex items-start gap-2.5">
                    <div
                      className="w-8 h-8 rounded-full flex items-center justify-center shrink-0"
                      style={{
                        background: c.targetFighter ? "rgba(232,180,0,0.12)" : "#f5f5f5",
                        border: c.targetFighter ? "1.5px solid rgba(232,180,0,0.4)" : "1.5px solid rgba(0,0,0,0.08)",
                        fontSize: "1rem",
                      }}
                    >
                      {c.avatar}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap mb-1">
                        <span style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: "0.88rem", color: "#333333" }}>
                          {c.user}
                        </span>
                        {c.targetFighter && (
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        ...
                        {c.targetFighter && (
                          <button
                            onClick={() => {
                              const f = allFighters.find((f) => (f.nickname || f.name) === c.targetFighter) ?? allFighters[0];
                              onOpenDonate(f);
                            }}
